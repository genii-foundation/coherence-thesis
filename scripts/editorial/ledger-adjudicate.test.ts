...
import {
  ...
  authorityKinds,
} from "./ledger-adjudicate";
import type { SentenceLedgerRecord } from "./sentence-ledger";
import {
  pendingStructureRouteOutcome,
  type StructureLedgerRecord,
} from "./structure-ledger";
import { sha256 } from "../manuscripts/shared";

const temporaryDirectories: string[] = [];
const reviewBinding = {
  sourceFile: "sources/manuscripts/test.md",
  sourceHash: "a".repeat(64),
  sentenceLedgerHash: "c".repeat(64),
  structureLedgerHash: "d".repeat(64),
};
...
function writeReviewRecord(directory: string): void {
  ...
  fs.writeFileSync(
    ...
    [
      ...
      "",
      `- Source file: \`${reviewBinding.sourceFile}\``,
      `- Source hash: \`${reviewBinding.sourceHash}\``,
      `- Initialized sentence ledger hash: \`${reviewBinding.sentenceLedgerHash}\``,
      `- Initialized structure ledger hash: \`${reviewBinding.structureLedgerHash}\``,
      `- Semantic review hash: \`${sha256(semantic)}\``,
      `- Literary review hash: \`${sha256(literary)}\``,
      `- Slop review hash: \`${sha256(slop)}\``,
      "",
    ].join("\n"),
  );
}
...
describe("editorial ledger adjudication", () => {
  it("requires all completed independent review artifacts", () => {
    ...
    expect(
      assertCompletedIndependentReviews(directory, reviewBinding).files,
    ).toHaveLength(4);
    fs.rmSync(path.join(directory, "literary-review.md"));
    expect(() =>
      assertCompletedIndependentReviews(directory, reviewBinding),
    ).toThrow(
  ...
  it("rejects failed or stale independent review evidence", () => {
    const directory = completedReviewDirectory();
    fs.writeFileSync(
      path.join(directory, "semantic-review.md"),
      "# Semantic review\n\n## Verdict\n\nFAIL because the claim changed.\n",
    );
    writeReviewRecord(directory);
    expect(() =>
      assertCompletedIndependentReviews(directory, reviewBinding),
    ).toThrow(/needs an explicit PASS verdict/);

    fs.appendFileSync(
      path.join(directory, "semantic-review.md"),
      "\nThis file changed after the review record was bound.\n",
    );
    expect(() =>
      assertCompletedIndependentReviews(directory, reviewBinding),
