...
describe("editorial structure ledger", () => {
  ...
  it("accounts for display quotations and attributions in first-wave sources", () => {
    const sourceFiles = [
      "editorial/sources/volumes/volume-02/manuscript.md",
      "editorial/sources/volumes/volume-03/manuscript.md",
    ];
    let displayLineCount = 0;
    for (const sourceFile of sourceFiles) {
      const source = fs.readFileSync(path.join(repoRoot, sourceFile), "utf8");
      const structureText = new Set(
        extractStructureUnits(source).map((unit) => unit.text),
      );
  ...
  it("uses the publishing parser to recognize bold headings in headingless sources", () => {
    const source = fs.readFileSync(
      path.join(
        repoRoot,
        "editorial/sources/volumes/volume-07/manuscript.md",
      ),
      "utf8",
    );
