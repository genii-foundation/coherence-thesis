...
import { ensureDir, repoRoot, sha256, writeUtf8 } from "../manuscripts/shared";
import {
...
function help(): string {
  return [
    ...
    "",
    "Usage:",
    "  npm run editorial:ledgers:init -- --base <sha> --current WORKTREE --source <source.md> --output <review-dir>",
    "",
...
export function runEditorialLedgerInitCli(
  ...
  try {
    ...
    if (
      ...
      !isCanonicalManuscriptPath(options.source) ||
    ...
    const baselineSource = sourcePathAtRef(baseCommit, options.source);
    const currentSource = sourcePathAtRef(options.current, options.source);
    ...
    const structureRecords = initializeStructureLedger(
      baselineSource,
      sourceAt(baseCommit, baselineSource),
      sourceAt(options.current, currentSource),
    );
