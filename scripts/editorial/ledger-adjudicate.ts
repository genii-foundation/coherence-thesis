...
export function runEditorialLedgerAdjudicateCli(
  ...
  try {
    ...
    if (
      ...
      !isCanonicalManuscriptPath(options.sourceFile)
    ) {
      throw new Error("--source must name a canonical manuscript in an editorial volume package.");
    }
    ...
    const baselineSourceFile = sourcePathAtRef(base, options.sourceFile);
    const currentSourceFile = sourcePathAtRef(
      options.current,
      options.sourceFile,
    );
    if (identity.sourceFile !== baselineSourceFile) {
      throw new Error(
        `The review ledgers belong to '${identity.sourceFile}', not '${options.sourceFile}'.`,
      );
    }
    const baselineSource = sourceAt(base, baselineSourceFile);
    if (sha256(baselineSource) !== identity.sourceHash) {
      throw new Error(
        "The review ledgers do not match the source file at the immutable baseline.",
      );
    }
    const currentSource = sourceAt(options.current, currentSourceFile);
