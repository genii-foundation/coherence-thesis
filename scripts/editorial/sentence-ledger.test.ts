...
import { extractEditorialSentences } from "./lint";
import {
...
import { repoRoot, sha256 } from "../manuscripts/shared";
...
describe("editorial sentence ledger", () => {
  ...
  it("rejects a ledger that drops a baseline citation attachment", () => {
    const citedBaseline: BaselineSentenceSection[] = [
      {
        sourceFile: "sources/manuscripts/test.md",
        sourceHash: "a".repeat(64),
        sectionId: "section",
        sentences: ["The sentence remains."],
    ...
    ).toThrow(/does not preserve its citation attachments/);
  });
  ...
  it("accounts for source sentences omitted from generated reader sections", () => {
    const sourcePaths = [
      {
        ...
        current: "editorial/sources/volumes/volume-01/manuscript.md",
      },
      {
        historical:
          "sources/manuscripts/coherence-thesis-vol2-wielding-intelligence.md",
        current: "editorial/sources/volumes/volume-02/manuscript.md",
      },
      {
        historical:
          "sources/manuscripts/coherence-thesis-vol3-the-providence-imperative.md",
        current: "editorial/sources/volumes/volume-03/manuscript.md",
      },
      {
        historical: "sources/manuscripts/coherence-thesis-master-ledger.md",
        current: "editorial/sources/corpus/master-ledger.md",
      },
    ];
    const sections = loadSentenceSections(
      "WORKTREE",
      sourcePaths.map(({ historical }) => historical),
    );

    for (const { current: sourceFile } of sourcePaths) {
    ...
    const master = sections.filter(
      (section) =>
        section.sourceFile === "editorial/sources/corpus/master-ledger.md",
    );
