...
import { extractEditorialSentenceRecords } from "./lint";
import {
  parseFrontmatter,
  repoRoot,
  sha256,
...
function generatedSectionFiles(ref: string): string[] {
  if (ref === "WORKTREE") {
    return markdownFilesIn(generatedSectionsRoot).map(
  ...
  return execFileSync(
    ...
    [
      ...
      ref,
      "--",
      repoRelative(generatedSectionsRoot),
      repoRelative(legacyPaths.generatedSectionsRoot),
    ],
...
export function loadSentenceSections(
  ...
  for (const sourceFile of extraSourceFiles) {
    ...
    const unpublished = sourceRecords.filter((record) => {
      const remaining = publishedCounts.get(record.text) ?? 0;
      if (remaining === 0) return true;
      publishedCounts.set(record.text, remaining - 1);
      return false;
    });
    if (unpublished.length > 0) {
      sections.push({
        ...
        sectionId: `${syntheticSourceSectionId(resolvedSourceFile)}:unpublished`,
        sentences: unpublished.map((record) => record.text),
        citationAttachments: unpublished.map(
          (record) => record.citationAttachments,
        ),
      });
    }
  ...
  for (const [sectionId, sectionRecords] of recordsBySection) {
    ...
    for (const [index, originalText] of section.sentences.entries()) {
      ...
      const expectedAttachments = [
        ...new Set(section.citationAttachments?.[index] ?? []),
      ].sort();
      const recordedAttachments = [
        ...new Set(record.citationAttachments),
      ].sort();
      if (
        JSON.stringify(recordedAttachments) !==
        JSON.stringify(expectedAttachments)
      ) {
        throw new Error(
          `${file}: section '${sectionId}' sentence ${index + 1} does not preserve its citation attachments.`,
        );
      }
    }
  }
...
function help(): string {
  return [
    ...
    "",
    "Usage:",
    "  npm run editorial:ledgers:validate -- --base <sha> --current WORKTREE --source <source.md> <ledger.jsonl>",
    "  npm run editorial:ledgers:validate -- --base <sha> --current WORKTREE --section <old-id> --current-section <new-id> --require-approved <ledger.jsonl>",
  ].join("\n");
}
...
export function runSentenceLedgerCli(args = process.argv.slice(2)): number {
  try {
    ...
    const baseline = baseCommit
      ...
      : undefined;
    const baselineSourceFiles = baseCommit
      ? options.sourceFiles.map((sourceFile) =>
          sourcePathAtRef(baseCommit, sourceFile),
        )
      : [];
    const current = options.current
      ? loadSentenceSections(options.current, options.sourceFiles)
      : undefined;
    const currentSourceFiles = options.current
      ? options.sourceFiles.map((sourceFile) =>
          sourcePathAtRef(options.current!, sourceFile),
        )
      : [];
