...
import { markdownHeading } from "../manuscripts/import-markdown";
import { normalizeNewlines, repoRoot, sha256 } from "../manuscripts/shared";
...
function isDisplayLine(line: string, beforeFirstHeading: boolean): boolean {
  ...
  return (
    ...
    /^\s*[\u2013\u2014]\s+\S/.test(line) ||
    /^\s*(?:\*\*[^*]+\*\*|\*[^*]+\*)\s*$/.test(line) ||
    /^(?:Written by|Edited by|Under|Published by|After|Adapted from)\b/i.test(
      trimmed,
    ) ||
...
export function extractStructureUnits(source: string): StructureUnit[] {
  ...
  const firstHeading = lines.findIndex((line) => markdownHeading(line));
...
function sourceAt(ref: string, sourceFile: string): string {
  ...
  if (ref === "WORKTREE") {
    return fs.readFileSync(path.join(repoRoot, resolvedSourceFile), "utf8");
  }
  return execFileSync("git", ["show", `${ref}:${resolvedSourceFile}`], {
    cwd: repoRoot,
    encoding: "utf8",
...
export function runStructureLedgerCli(args = process.argv.slice(2)): number {
  try {
    if (args.includes("--help") || args.includes("-h")) {
      console.log(
        "npm run editorial:structure-ledger -- --base <sha> --current WORKTREE --source <source.md> [--require-approved] <ledger.jsonl>",
      );
      return 0;
    }
