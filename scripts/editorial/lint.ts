...
import { masterLedgerPath } from "../repository/paths";
...
export type EditorialSentenceRecord = {
  text: string;
...
function sentenceRecords(value: string): EditorialSentenceRecord[] {
  const citations: Array<{ token: string; display: string; url: string }> = [];
  const addCitation = (display: string, url: string): string => {
    const token = `\uE000citation${citations.length}\uE001`;
    citations.push({ token, display, url });
    return token;
  };
  let tokenized = value.replace(
    /\[([^\]]+)]\(([^)\s]+)(?:\s+["'][^"']*["'])?\)/g,
    (_match, label: string, url: string) => addCitation(label, url),
  );
  tokenized = tokenized.replace(/https?:\/\/[^\s)>]+/g, (rawUrl) => {
    const trailing = rawUrl.match(/[.,!?;:]+$/)?.[0] ?? "";
    const url = trailing ? rawUrl.slice(0, -trailing.length) : rawUrl;
    return `${addCitation(url, url)}${trailing}`;
  });
  const plain = plainSentenceText(tokenized);
  if (!plain) return [];
  const extracted =
    plain.match(/[^.!?]+(?:[.!?]+["'\u2019\u201d)]*|$)/g)?.map((sentence) => sentence.trim()) ??
    [];
  return extracted.filter(Boolean).map((sentence) => {
    const attachments: string[] = [];
    let text = sentence;
    for (const citation of citations) {
      if (!text.includes(citation.token)) continue;
      text = text.replaceAll(citation.token, citation.display);
      attachments.push(citation.url);
    }
    return {
      text: text.replace(/\s+/g, " ").trim(),
      citationAttachments: [...new Set(attachments)],
    };
  });
}

function sentences(value: string): string[] {
  return sentenceRecords(value).map((record) => record.text);
}

export function extractEditorialSentenceRecords(
  source: string,
): EditorialSentenceRecord[] {
  const lines = normalizeNewlines(source).split("\n");
  const extracted: EditorialSentenceRecord[] = [];
  ...
  const flush = () => {
    ...
    extracted.push(...sentenceRecords(paragraphLines.join(" ")));
    paragraphLines = [];
  };
...
export function extractEditorialSentences(source: string): string[] {
  return extractEditorialSentenceRecords(source).map((record) => record.text);
}
...
export function resolveEditorialFiles(options: Pick<EditorialCliOptions, "paths" | "volumes">): string[] {
  ...
  if (selected.size === 0) {
    ...
    if (fs.existsSync(masterLedgerPath)) selected.add(masterLedgerPath);
  }
...
export function editorialHelp(): string {
  return [
    ...
    "  npm run editorial:lint -- <file-or-directory> --strict",
    "",
    "Options:",
