...
describe("editorial link preservation", () => {
  ...
  it("matches every predecessor owned by one reviewed merged lineage", () => {
    const first = section({
      id: "old-first",
      href: "/old-first/",
      hash: "first",
      text: "The first former section.",
    });
    const second = section({
      id: "old-second",
      href: "/old-second/",
      hash: "second",
      text: "The second former section.",
    });
    const merged = section({
      id: "reviewed-merge",
      href: "/reviewed-merge/",
      hash: "merged",
      text: "One reviewed section now carries both arguments.",
    });
    const plan = planLinkPreservation({
      ...
      existingSectionLineage: {
        ...
        sections: [
          {
            currentSectionId: "reviewed-merge",
  ...
  it("keeps a four-figure historical route set out of durable section aliases", () => {
    const previous = section({
      id: "retired",
      href: "/manuscripts/1/legacy/route-0/",
      hash: "old",
      text: "The former section.",
    });
    const current = section({
      id: "current",
      href: "/manuscripts/1/part/chapter/current/",
      hash: "new",
      text: "The revised section.",
    });
    const historicalAliases: CompiledCatalog["aliases"] = Array.from(
      { length: 1_100 },
      (_, index) => ({
        sourceHref: `/manuscripts/1/legacy/route-${index}/`,
        ...
        sourceRoute: {
          ...
          partId: "legacy",
          chapterId: "legacy",
          sectionId: `retired-${index}`,
        },
      }),
    );
    const routeLedger = {
      version: 2,
      routes: historicalAliases.map((alias) => ({
        href: alias.sourceHref,
        kind: "section-alias" as const,
        targetContinuityIds: ["retired"],
      })),
    };

    const plan = planLinkPreservation({
      previous: catalog([previous]),
      current: catalog([current], { aliases: historicalAliases }),
      ...
      existingSectionLineage: {
        ...
        sections: [
          {
            currentSectionId: "current",
            continuityIds: ["retired"],
            historicalSectionIds: ["retired"],
            progressContinuityGroups: [["retired"]],
          },
        ],
      },
    });
    ...
    expect(plan.lineage).toEqual([
      expect.objectContaining({
        previousSectionId: "retired",
        currentSectionId: "current",
        reason: "established-lineage",
      }),
    ]);
    expect(historicalAliases).toHaveLength(1_100);
    expect(plan.sectionAliases.aliases).toHaveLength(0);
  });

  it("stops on a missing reviewed target until an explicit map replaces it", () => {
    const previous = section({
      id: "retired",
      href: "/retired/",
      hash: "old",
    ...
    const reviewedLineage = {
      ...
      sections: [
        {
          currentSectionId: "reviewed-name-that-no-longer-exists",
          continuityIds: ["retired"],
          historicalSectionIds: ["retired"],
          progressContinuityGroups: [["retired"]],
        },
    ...
    const blocked = planLinkPreservation({
      previous: catalog([previous]),
      current: catalog([renamed]),
      existingSectionLineage: reviewedLineage,
    });
    expect(blocked.lineage).toEqual([]);
    expect(blocked.unresolved).toContainEqual(
      expect.objectContaining({
        previousSectionId: "retired",
        message: expect.stringContaining("is not in the current catalog"),
      }),
    );
    ...
    const resolved = planLinkPreservation({
      ...
      current: catalog([renamed]),
      existingSectionLineage: reviewedLineage,
      explicitMappings: new Map([["retired", "renamed-again"]]),
    });
    expect(resolved.unresolved).toEqual([]);
    expect(resolved.lineage).toContainEqual(
      expect.objectContaining({
        previousSectionId: "retired",
        currentSectionId: "renamed-again",
        reason: "explicit",
      }),
    );
  });

  it("rejects ambiguous reviewed ownership unless an explicit map wins", () => {
    const previous = section({
      id: "retired",
      href: "/retired/",
      hash: "old",
    ...
    const first = section({
      id: "first-candidate",
      href: "/first-candidate/",
      hash: "first",
      text: "The first candidate.",
    });
    const second = section({
      id: "second-candidate",
      href: "/second-candidate/",
      hash: "second",
      text: "The second candidate.",
    });
    const reviewedLineage = {
      version: 1,
      sections: [
        {
          currentSectionId: "first-candidate",
          continuityIds: ["retired"],
          historicalSectionIds: ["retired"],
          progressContinuityGroups: [["retired"]],
        },
        {
          currentSectionId: "second-candidate",
          continuityIds: ["retired"],
          historicalSectionIds: ["retired"],
          progressContinuityGroups: [["retired"]],
        },
    ...
    const ambiguous = planLinkPreservation({
      previous: catalog([previous]),
      current: catalog([first, second]),
      existingSectionLineage: reviewedLineage,
    });
    expect(ambiguous.unresolved).toContainEqual(
      expect.objectContaining({
        previousSectionId: "retired",
        message: expect.stringContaining("multiple current sections"),
      }),
    );

    const explicit = planLinkPreservation({
      previous: catalog([previous]),
      current: catalog([first, second]),
      existingSectionLineage: reviewedLineage,
      explicitMappings: new Map([["retired", "second-candidate"]]),
    });
    expect(explicit.unresolved).toEqual([]);
    expect(explicit.lineage).toContainEqual(
      expect.objectContaining({
        currentSectionId: "second-candidate",
        reason: "explicit",
      }),
    ...
    expect(
      explicit.sectionLineage.sections.find(
        (entry) => entry.currentSectionId === "second-candidate",
      )?.continuityIds,
    ).toContain("retired");
    expect(
      explicit.sectionLineage.sections.find(
        (entry) => entry.currentSectionId === "first-candidate",
      )?.continuityIds,
    ).not.toContain("retired");
  });

  it("parses reviewed replacement file flags", () => {
    const options = parseLinkArgs([
      "--reviewed-lineage",
      "artifacts/reviewed-lineage.json",
      "--reviewed-route-aliases",
      "/tmp/reviewed-routes.json",
      "--write",
    ]);

    expect(options.reviewedLineagePath).toBe(
      "artifacts/reviewed-lineage.json",
    );
    expect(options.reviewedRouteAliasesPath).toBe(
      "/tmp/reviewed-routes.json",
    );
