...
type CliOptions = {
  ...
  write: boolean;
  format: "text" | "json";
  reviewedLineagePath?: string;
  reviewedRouteAliasesPath?: string;
...
function matchLineage(
  ...
  const establishedByIdentity = new Map<string, Set<string>>();
  for (const entry of existingLineage.sections) {
    for (const identity of [
      ...
      const owners = establishedByIdentity.get(identity) ?? new Set<string>();
      owners.add(entry.currentSectionId);
      establishedByIdentity.set(identity, owners);
    }
  }
  const matches = new Map<string, SectionLineageMatch>();
  const automaticallyUsedCurrent = new Set<string>();
  const blockedByReviewedLineage = new Set<string>();
  ...
  for (const previousSection of previous.sections) {
    ...
    const reviewedOwners = new Set(
      previousIdentities.flatMap((identity) => [
        ...(establishedByIdentity.get(identity) ?? []),
      ]),
    );
    if (reviewedOwners.size === 0) continue;
    if (reviewedOwners.size > 1) {
      blockedByReviewedLineage.add(previousSection.sectionId);
      unresolved.push({
        previousSectionId: previousSection.sectionId,
        sourceHref: previousSection.href,
        message:
          `Reviewed lineage assigns this predecessor to multiple current sections: ${[
            ...reviewedOwners,
          ].sort().join(", ")}. Supply one explicit section mapping.`,
        candidates: [...reviewedOwners]
          .sort()
          .map((sectionId) => ({ sectionId, score: 1 })),
      });
      continue;
    }
    const currentSectionId = [...reviewedOwners][0]!;
    const currentSection = currentById.get(currentSectionId);
    if (!currentSection) {
      blockedByReviewedLineage.add(previousSection.sectionId);
      unresolved.push({
        previousSectionId: previousSection.sectionId,
        sourceHref: previousSection.href,
        message:
          `Reviewed lineage successor '${currentSectionId}' is not in the current catalog. Supply an explicit section mapping.`,
      });
      continue;
    }
    add(
  ...
  for (const previousSection of previous.sections) {
    if (
      matches.has(previousSection.sectionId) ||
      blockedByReviewedLineage.has(previousSection.sectionId)
    ) {
      continue;
    }
...
function buildSectionLineageConfig(
  ...
  const reviewedOwnerOverrides = new Map(explicitContinuityMappings);
  for (const match of matches) {
    if (match.reason !== "explicit") continue;
    const predecessor = previousById.get(match.previousSectionId);
    if (!predecessor) continue;
    for (const identity of [
      ...compatibleContinuityIds(predecessor),
      ...(predecessor.legacySectionIds ?? []),
      predecessor.sectionId,
    ]) {
      if (!reviewedOwnerOverrides.has(identity)) {
        reviewedOwnerOverrides.set(identity, match.currentSectionId);
      }
    }
  }
  const addClaim = (
  ...
  for (const currentSection of current.sections) {
    ...
    for (const id of established?.continuityIds ?? []) {
      const reviewedOwner = reviewedOwnerOverrides.get(id);
      if (reviewedOwner && reviewedOwner !== currentSection.sectionId) continue;
      addClaim(
    ...
    for (const group of established?.progressContinuityGroups ?? []) {
      if (
        group.some((id) => {
          const reviewedOwner = reviewedOwnerOverrides.get(id);
          return reviewedOwner && reviewedOwner !== currentSection.sectionId;
        })
      ) {
        continue;
      }
...
export function loadCatalogAtGitRef(ref: string): CompiledCatalog {
  ...
  let source: string;
  try {
    source = execFileSync("git", ["show", `${ref}:${relativeCatalogPath}`], {
      cwd: repoRoot,
      encoding: "utf8",
    ...
    source = materializeCatalogAtGitRef(ref);
  }
...
function materializeCatalogAtGitRef(ref: string): string {
  ...
  const temporaryRoot = fs.mkdtempSync(
    path.join(os.tmpdir(), "coherence-link-baseline-"),
  );
  ...
  try {
    ...
    const dependencyRoot = fs.realpathSync(path.join(repoRoot, "node_modules"));
    fs.symlinkSync(dependencyRoot, path.join(worktreePath, "node_modules"), "dir");
    const require = createRequire(import.meta.url);
    const tsxCliPath = require.resolve("tsx/cli");
    const runTsx = (args: string[]) =>
      execFileSync(process.execPath, [tsxCliPath, ...args], {
        cwd: worktreePath,
        encoding: "utf8",
    ...
    runTsx(["scripts/manuscripts/import-markdown.ts"]);
    return runTsx([
      "--eval",
      "import { buildCatalog } from './scripts/manuscripts/shared.ts'; process.stdout.write(JSON.stringify(buildCatalog()));",
    ]);
  } finally {
    if (worktreeAdded) {
      try {
        ...
        // The temporary directory is removed below. A later `git worktree prune`
        // can discard metadata if Git was interrupted during cleanup.
      }
    }
...
function reviewedConfigPath(inputPath: string): string {
  return path.isAbsolute(inputPath)
    ? inputPath
    : path.resolve(process.cwd(), inputPath);
}

function readReviewedJson(inputPath: string, label: string): unknown {
  const resolvedPath = reviewedConfigPath(inputPath);
  let source: string;
  try {
    source = fs.readFileSync(resolvedPath, "utf8");
  } catch (error) {
    throw new Error(
      `${label} '${resolvedPath}' could not be read: ${
  ...
  try {
    ...
    throw new Error(
      `${label} '${resolvedPath}' is not valid JSON: ${
        error instanceof Error ? error.message : String(error)
      }`,
    );
  }
...
function readReviewedLineage(inputPath: string): SectionLineageConfig {
  const value = readReviewedJson(inputPath, "Reviewed lineage");
  if (
    !value ||
    typeof value !== "object" ||
    ...
    throw new Error("Reviewed lineage must be a version 1 section lineage file.");
  }
  const config = value as SectionLineageConfig;
  const currentIds = new Set<string>();
  for (const entry of config.sections) {
    if (
      !entry ||
      typeof entry.currentSectionId !== "string" ||
      !Array.isArray(entry.continuityIds) ||
      entry.continuityIds.some((id) => typeof id !== "string" || !id) ||
      !Array.isArray(entry.historicalSectionIds) ||
      entry.historicalSectionIds.some((id) => typeof id !== "string" || !id) ||
      (entry.progressContinuityGroups !== undefined &&
        (!Array.isArray(entry.progressContinuityGroups) ||
          entry.progressContinuityGroups.some(
            (group) =>
              !Array.isArray(group) ||
              group.some((id) => typeof id !== "string" || !id),
          )))
    ) {
      throw new Error("Reviewed lineage contains an invalid section entry.");
    }
    if (currentIds.has(entry.currentSectionId)) {
      throw new Error(
        `Reviewed lineage repeats current section '${entry.currentSectionId}'.`,
      );
    }
...
function readReviewedRouteAliases(inputPath: string): RouteAliasConfig {
  const value = readReviewedJson(inputPath, "Reviewed route aliases");
  if (
    !value ||
    typeof value !== "object" ||
    (value as { version?: unknown }).version !== 1 ||
    ...
    throw new Error(
      "Reviewed route aliases must be a version 1 structural route alias file.",
    );
  }
  const config = value as RouteAliasConfig;
  const targetsBySource = new Map<string, string>();
  for (const alias of config.aliases) {
    if (
      !alias ||
      typeof alias.sourceHref !== "string" ||
      !alias.sourceHref ||
      typeof alias.targetHref !== "string" ||
      !alias.targetHref ||
      (alias.note !== undefined && typeof alias.note !== "string")
    ) {
      throw new Error("Reviewed route aliases contain an invalid alias entry.");
    }
    const existingTarget = targetsBySource.get(alias.sourceHref);
    if (existingTarget && existingTarget !== alias.targetHref) {
      throw new Error(
        `Reviewed route '${alias.sourceHref}' has conflicting targets '${existingTarget}' and '${alias.targetHref}'.`,
      );
    }
    if (existingTarget) {
      throw new Error(
        `Reviewed route aliases repeat source '${alias.sourceHref}'.`,
      );
    }
    targetsBySource.set(alias.sourceHref, alias.targetHref);
  }
  return config;
}
...
export function parseLinkArgs(args: string[]): CliOptions {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (arg === "--help" || arg === "-h") options.help = true;
      ...
      index += 1;
    } else if (arg === "--reviewed-lineage") {
      if (!value || value.startsWith("--")) {
        throw new Error("--reviewed-lineage requires a path.");
      }
      options.reviewedLineagePath = value;
      index += 1;
    } else if (arg === "--reviewed-route-aliases") {
      if (!value || value.startsWith("--")) {
        throw new Error("--reviewed-route-aliases requires a path.");
      }
      options.reviewedRouteAliasesPath = value;
      index += 1;
    } else if (arg === "--map") {
...
export function linkHelp(): string {
  return [
    ...
    "  npm run manuscripts:preserve-links -- --reviewed-lineage path/to/lineage.json --reviewed-route-aliases path/to/route-aliases.json --write",
    "",
    "Options:",
    "  --base <git-ref>        Catalog baseline. Default: HEAD.",
    "  --reviewed-lineage <path>  Use a reviewed replacement lineage as the planning input.",
    "  --reviewed-route-aliases <path>  Use reviewed replacement structural aliases as the planning input.",
...
export function runLinkCli(args = process.argv.slice(2)): number {
  try {
    ...
    const canonicalLineage = readSectionLineage();
    const canonicalSectionAliases = readAliasConfig();
    const canonicalRouteAliases = readRouteAliasConfig();
    const reviewedLineage = options.reviewedLineagePath
      ? readReviewedLineage(options.reviewedLineagePath)
      : canonicalLineage;
    const reviewedRouteAliases = options.reviewedRouteAliasesPath
      ? readReviewedRouteAliases(options.reviewedRouteAliasesPath)
      : canonicalRouteAliases;
    const current = buildCatalog(manuscriptRoot, {
      aliasConfig: { version: 1, aliases: [] },
      routeAliasConfig: { version: 1, aliases: [] },
      sectionLineage: reviewedLineage,
    });
    const plan = planLinkPreservation({
      previous,
      current,
      existingSectionAliases: canonicalSectionAliases,
      existingRouteAliases: reviewedRouteAliases,
      existingRouteLedger: readRouteLedger(),
      existingSectionLineage: reviewedLineage,
    ...
    const canonicalRouteDiff = diffRouteAliases(
      sortedRouteAliases(canonicalRouteAliases.aliases),
      plan.routeAliases.aliases,
    );
    plan.addedRouteAliases = canonicalRouteDiff.added;
    plan.updatedRouteAliases = canonicalRouteDiff.updated;
    plan.removedRouteAliases = canonicalRouteDiff.removed;
    plan.changed =
      !sameJson(
        { version: 1, sections: canonicalLineage.sections },
        plan.sectionLineage,
      ) ||
      !sameJson(
        sortedSectionAliases(canonicalSectionAliases.aliases),
        plan.sectionAliases.aliases,
      ) ||
      !sameJson(
        sortedRouteAliases(canonicalRouteAliases.aliases),
        plan.routeAliases.aliases,
