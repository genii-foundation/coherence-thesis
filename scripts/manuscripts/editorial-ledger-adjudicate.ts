...
import { repoRoot, sha256, writeUtf8 } from "./shared";
...
type IndependentReviewBinding = {
  sourceFile: string;
  sourceHash: string;
  sentenceLedgerHash: string;
  structureLedgerHash: string;
};
...
function reviewArtifactVerdict(source: string): "PASS" | "FAIL" | null {
  const lines = source.split(/\r?\n/);
  for (let index = 0; index < lines.length; index += 1) {
    const line = lines[index]?.trim() ?? "";
    const inline = line.match(/^(?:[-*]\s*)?Verdict:\s*(PASS|FAIL)\b/i);
    if (inline) return inline[1]?.toUpperCase() as "PASS" | "FAIL";
    if (!/^#{1,6}\s+Verdict\s*$/i.test(line)) continue;
    for (const candidate of lines.slice(index + 1)) {
      const value = candidate.trim();
      if (!value) continue;
      const verdict = value.match(/^(PASS|FAIL)\b/i);
      return verdict
        ? (verdict[1]?.toUpperCase() as "PASS" | "FAIL")
        : null;
    }
  }
  return null;
...
function reviewField(source: string, label: string): string | null {
...
export function assertCompletedIndependentReviews(
  reviewDirectory: string,
  binding: IndependentReviewBinding,
  ...
  const reviewSources = new Map(
    files.slice(1).map((file) => [file, fs.readFileSync(file, "utf8")]),
  );
  const expectedFields = new Map<string, string>([
    ["Source file", binding.sourceFile],
    ["Source hash", binding.sourceHash],
    ["Initialized sentence ledger hash", binding.sentenceLedgerHash],
    ["Initialized structure ledger hash", binding.structureLedgerHash],
    [
      "Semantic review hash",
      sha256(reviewSources.get(files[1]!) ?? ""),
    ],
    [
      "Literary review hash",
      sha256(reviewSources.get(files[2]!) ?? ""),
    ],
    ["Slop review hash", sha256(reviewSources.get(files[3]!) ?? "")],
  ]);
  for (const [label, expected] of expectedFields) {
    const actual = reviewField(mainReview, label);
    if (actual !== expected) {
      throw new Error(
        `Independent review is stale or incomplete: review.md ${label} does not match the current review batch.`,
      );
    }
  }
  for (const file of files.slice(1)) {
    const verdict = reviewArtifactVerdict(reviewSources.get(file) ?? "");
    if (verdict !== "PASS") {
      throw new Error(
        `Independent review is incomplete: ${path.basename(file)} needs an explicit PASS verdict.`,
      );
    }
  }
...
function singleLedgerIdentity(
  sentences: SentenceLedgerRecord[],
  structure: StructureLedgerRecord[],
): Pick<IndependentReviewBinding, "sourceFile" | "sourceHash"> {
  const identities = new Set(
    [...sentences, ...structure].map((record) =>
      JSON.stringify([record.sourceFile, record.sourceHash]),
    ),
  );
  if (identities.size !== 1) {
    throw new Error(
      "Editorial adjudication requires one source file and one immutable source hash per review batch.",
    );
  }
  const [sourceFile, sourceHash] = JSON.parse(
    [...identities][0] ?? "[]",
  ) as [string, string];
...
function sourceAt(ref: string, sourceFile: string): string {
  return ref === "WORKTREE"
    ...
    : execFileSync("git", ["show", `${ref}:${sourceFile}`], {
        cwd: repoRoot,
        encoding: "utf8",
...
function parseArgs(args: string[]): CliOptions {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (arg === "--write") options.write = true;
      ...
      options.base = optionValue(args, index, arg);
      index += 1;
    } else if (arg === "--current") {
      options.current = optionValue(args, index, arg);
      index += 1;
    } else if (arg === "--source") {
      options.sourceFile = optionValue(args, index, arg);
      index += 1;
    } else {
...
function help(): string {
  return [
    ...
    "",
    "Usage:",
    "  npm run manuscripts:editorial-ledgers:adjudicate -- --base <sha> --current WORKTREE --source <source.md> --review editorial/reviews/<volume>/<batch> [--write]",
    "",
...
export function runEditorialLedgerAdjudicateCli(
  ...
  try {
    ...
    if (!options.base || !options.current || !options.sourceFile) {
      throw new Error(
        "Adjudication requires --base, --current, and --source so the ledgers can be reconstructed before any status changes.",
      );
    }
    if (
      path.isAbsolute(options.sourceFile) ||
      !options.sourceFile.startsWith("sources/manuscripts/") ||
      !options.sourceFile.endsWith(".md")
    ) {
    ...
    const sentenceSource = fs.readFileSync(sentenceFile, "utf8");
    const structureSource = fs.readFileSync(structureFile, "utf8");
    const pendingSentences = parseSentenceLedger(sentenceSource, sentenceFile);
    const pendingStructure = parseStructureLedger(
      structureSource,
      structureFile,
    );
    const identity = singleLedgerIdentity(
      pendingSentences,
      pendingStructure,
    );
    if (identity.sourceFile !== options.sourceFile) {
      throw new Error(
        `The review ledgers belong to '${identity.sourceFile}', not '${options.sourceFile}'.`,
      );
    }
    const base = resolveImmutableEditorialBase(options.base);
    const baselineSource = sourceAt(base, options.sourceFile);
    if (sha256(baselineSource) !== identity.sourceHash) {
      throw new Error(
        "The review ledgers do not match the source file at the immutable baseline.",
      );
    }
    const currentSource = sourceAt(options.current, options.sourceFile);
    const baselineSections = loadSentenceSections(base, [options.sourceFile]);
    const currentSections = loadSentenceSections(options.current, [options.sourceFile]);
    ...
    assertCompletedIndependentReviews(reviewDirectory, {
      ...identity,
      sentenceLedgerHash: sha256(sentenceSource),
      structureLedgerHash: sha256(structureSource),
    });
    const sentences = adjudicateSentenceRecords(pendingSentences);
    const structure = adjudicateStructureRecords(pendingStructure);
