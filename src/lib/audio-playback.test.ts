...
describe("browser speech provider", () => {
  ...
  describe("with a speech engine present", () => {
    ...
    it("does not start lazy speech after it is paused while loading", async () => {
      const provider = createBrowserSpeechProvider();
      ...
      provider.pause();
      resolveText?.("Section A. Complete body.");
      await text;
...
describe("hosted clip provider", () => {
  ...
  it("starts hosted media before lazy canonical text finishes loading", async () => {
    ...
    let resolveText: ((text: string) => void) | undefined;
    const canonicalText = new Promise<string>((resolve) => {
      resolveText = resolve;
    });
    const provider = createHostedClipProvider({
      version: 1,
