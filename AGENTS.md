...
- Historical version links are best effort. Associate a card with the successful public Vercel production deployment for that exact full commit SHA, verify cached or discovered URLs before publishing them, and omit links that are unavailable. The current Vercel production build may trust its own exact deployment URL because it cannot be reached until that build finishes.
- Deployment link discovery must never weaken complete history generation or its freshness checks. Keep remote checks bounded. CI preserves the checked deployment mappings instead of refreshing optional links during required gates. A missing deployment is allowed. A missing commit is not.
