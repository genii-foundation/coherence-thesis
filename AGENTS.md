...
- Production builds must generate history through the exact deployed `main` SHA. They first expand a shallow checkout from the existing `origin`, then use the GitHub API as a fallback. If complete Git history and GitHub are both unavailable, production must fail without replacing the last good deployment. It must never publish a green but stale Updates page.
