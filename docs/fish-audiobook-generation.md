...
The timing sidecar maps Fish word alignment segments to character ranges in the exact canonical audio text. Exact matches are retained. Isolated normalization differences, commonly spoken numbers, are interpolated between exact neighbors. Generation fails if fewer than 90 percent of canonical words align exactly.
...
```bash
FISH_AUDIO_API_KEY=<from-secret-store> npm run audio:fish -- \
  --mode sample \
...
Publish to a new immutable version path. The publisher uploads audio and timing sidecars together and refuses stale, duplicate, incomplete, or malformed mappings.
