...
5. Capture import context while working:
...
7. Run `npm run build` when route data, overview references, or generated catalog data changed.
8. Review the final diff before staging. Confirm generated files are expected, public link preservation is handled, no import report surprise is ignored, and unrelated local changes are left alone.
9. Commit with an `edit:` Conventional Commit title, push the branch, and open or update a focused pull request. If the user explicitly requested direct main work, commit directly on `main` and do not open a pull request unless asked.
