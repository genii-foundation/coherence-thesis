---
name: coherence-admin-editorial-debt-guide
...
- Route literary, structural, and terminology work to `coherence-admin-editorial-review`.
- Treat canon, logical, and promise debt as an author or project decision before routing approved prose work to `coherence-admin-editorial-review`.
- Establish factual and citation claims from primary or authoritative evidence before editing prose.
- Route link and continuity work to `coherence-utility-manuscript-publish`.
- Route technical and reader behavior work to `coherence-utility-build-feature`.
...
- Use `coherence-utility-editorial-debt` for every durable register mutation.
...
Resolution proof must state the outcome, result for every criterion, exact files and revision, validation, approval, residual risk, and related active debt. Record it through `coherence-utility-editorial-debt`. A `resolved` ticket on a pull request branch is a complete closure candidate; merge makes it canonical. If any criterion is unmet or evidence has gone stale, keep the ticket active in that same pull request.
